...
describe('calcularPnl — critérios de aceite', () => {
  ...
  it('3. toggle de frustrados ligado, sem custos variáveis: lucro 3.101,08', () => {
    const r = calcularPnl([dia], [], periodo, { considerarFrustrados: true });
    expect(r.lucro_real).toBe(310_108); // R$ 3.101,08
    // e o valor "com frustrados" bate mesmo com o toggle desligado:
    const off = calcularPnl([dia], [], periodo);
    expect(off.lucro_real_com_frustrados).toBe(310_108);
  });
