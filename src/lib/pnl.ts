...
import { COMISSAO_COBRANCA, COMISSAO_VENDEDOR, TAXA_PLATAFORMA, custosDePedidos } from '@/lib/custosConfig';
import {
  diasDoPeriodo,
  diasInclusivos,
  diasNoMes,
...
export function calcularPnl(
  ...
  if (agg && agg.qtd_agendados > 0) {
    ...
    frete = cc.frete;
    taxas_plataforma = agg.qtd_pagamentos > 0 ? reaisToCents(TAXA_PLATAFORMA) : 0;
